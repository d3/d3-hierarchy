...
Unlike [*node*.sum](#node_sum), the *compare* function is passed two [nodes](#hierarchy-node) rather than two nodes’ [data](#node_data). For example, if the data has a value property, this sorts nodes by the aggregate [value](#node_value) of the node and all its descendants:
